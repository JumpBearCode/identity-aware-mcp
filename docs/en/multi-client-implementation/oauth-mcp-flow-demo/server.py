#!/usr/bin/env python3
"""
server.py — A teaching backend that "simulates a statically registered OAuth client", supporting two flows:

  ● mcpproxy flow: client sends resource → goes through /mcpproxy proxy (proxy strips resource then forwards to Entra).
  ● mcp flow    : client does not send resource → PRM points directly to Entra, client connects directly to Entra (like VS Code).

It is three things at once:
  1. A static file server: serves the visualization pages under public/.
  2. A real OAuth public client (static client_id 49af5fc1 + PKCE + loopback callback).
  3. A "packet sniffer" for each of the two flows (14 / 11 steps respectively): captures every real request/response into a structure the frontend can render.

Why it must run on :8080 — the redirect whitelist registered in Entra for client 49af5fc1 only includes
`http://localhost:8080/callback`, so this process must listen on 8080 so the login callback can land back here.

Uses only the Python standard library, zero pip dependencies: `python3 server.py` is enough.
Corresponding document: docs/en/multi-client-implementation/implementation-notes-plan-a-mcpproxy-resource-stripping-proxy.md
"""
...
# ── Constants (consistent with project configuration) ─────────────────────────
BASE = os.environ.get(
    "MCP_BASE_URL",
...
# Entra (direct flow hits these directly)
...
# ── In-memory state for single-user teaching scenario ─────────────────────────
RUN = {
...
def mcp_post(payload: dict, token: str, session_id: str | None, mcp_url: str, want_response=True):
    """Send a JSON-RPC to a streamable-HTTP MCP endpoint, handling both SSE and JSON responses."""
    headers = {
...
def _trim_openid(body: bytes) -> str:
    """Entra's openid-configuration has dozens of fields; for teaching we keep only the key ones."""
    try:
    ...
    trimmed["//registration_endpoint"] = full.get("registration_endpoint", "field missing → DCR not advertised")
...
# ── Steps 1–4: real discovery (two flows diverge) ─────────────────────────────
def capture_discovery(flow: str) -> dict:
    ...
    # 2: real 401 (GET also returns 401, with the same WWW-Authenticate)
    ...
    # 4: AS metadata — proxy flow asks the proxy, direct flow asks Entra
    if flow == "mcpproxy":
...
# ── Step 5(–6): construct authorization request ───────────────────────────────
def capture_authorize(flow: str) -> dict:
    ...
    if flow == "mcpproxy":
        # Go through proxy /authorize (with resource), and capture the proxy's real 302 (resource already stripped)
        query = [
    ...
    # Direct flow: hit Entra /authorize directly (no resource sent), no proxy 302 hop
    query = [
        ("response_type", "code"), ("client_id", CLIENT_ID), ("redirect_uri", REDIRECT_URI),
...
# ── After login: exchange token + tools/list (two flows diverge) ──────────────
...
def _proxy_after_login(code, verifier, state, steps):
    ...
    # Real token exchange (hit proxy /token; proxy internally strips resource and forwards to Entra)
    ...
    # 10: proxy internally forwards to Entra (we cannot observe it; shown for illustration; resource already stripped)
    ...
    steps["11"] = {"response": {"status": st, "statusText": "OK" if st == 200 else "",
        ...
        "decoded": {"title": "Decoded claims of the real access_token (signature not verified, for teaching)", "claims": claims} if claims else None}}
...
def _direct_after_login(code, verifier, state, steps):
    ...
    steps["8"] = {"request": {"method": "POST", "url": ENTRA_TOKEN,
        ...
        "form": [["grant_type", "authorization_code"], ["code", redact(code, 20)], ["code_verifier", redact(verifier, 16)],
                 ["client_id", CLIENT_ID], ["redirect_uri", REDIRECT_URI]]}}
    # Real token exchange (hit Entra /token directly, no resource)
...
class Handler(BaseHTTPRequestHandler):
    def log_message(self, *a):  # Keep quiet; and never print URLs that may contain code/token
        pass
    ...
    def _callback(self):
        ...
        if not code or state != RUN.get("state"):
            with LOCK:
                RUN["phase"] = "error"; RUN["error"] = "state mismatch or missing code"
            return self._html(_result_page(False, "state mismatch or missing code (possibly an expired login)"))
        try:
...
def _result_page(ok: bool, msg: str | None) -> str:
    ...
    title = "✓ Login succeeded, token + tools/list captured" if ok else "✕ An error occurred"
    body = "Go back to the teaching page to see the animated real data for each post-login step. You can close this window now." if ok else (msg or "")
    return f"""<!doctype html><html lang=zh-CN><head><meta charset=utf-8>
<title>Login Callback</title><style>
...
def main():
    ...
    print("┌───────────────────────────────────────────────────────────────┐")
    print("│  MCP · OAuth Authorization Flow Visualization Teaching App   │")
    print("│  (mcpproxy ↔ mcp two flows)                                  │")
    print("├───────────────────────────────────────────────────────────────┤")
    print(f"│  Open:   http://localhost:{PORT}")
    print(f"│  Target: {PROXY_URL}  /  {MCP_URL}")
    print(f"│  Callback: {REDIRECT_URI}  (must be in Entra whitelist)")
    print("│  Stop:   Ctrl-C")
    print("└───────────────────────────────────────────────────────────────┘")
    try:
