...
> Microsoft Entra v2 端点（不接受 `resource` 参数）之间的协议级不兼容。** 2026-03 Entra 上线强制校验后，
...
|---|---|---|
| 出身 | OAuth 2.0 / OIDC，Entra **v2 原生主力** | OAuth v1 的 `resource`，被 **RFC 8707**（Resource Indicators）在 v2 时代复活 |
...
| Entra v2 怎么处理 | **主力**：从 scope 前缀推出 token 的 `aud`；`.default` 写法 `api://<appid>/.default` | **不认**：v2 是 scope-centric、不实现 RFC 8707，带了就拒（AADSTS9010010） |
...
URI**（不是租户验证过的域名），所以也**没法"把两者对齐"**。加上 Entra v2 本就不接受任何 `resource` 参数
——**唯一出路是让请求里干脆不带 `resource`。**
...
没有单一元凶，是三方叠加的**规范 vs 提供方**撞车：

| 角色 | 责任 |
|---|---|
| **MCP 规范** | **根源**：**强制（MUST）** 客户端带 RFC 8707 `resource`，却**没给"AS 不支持就回退"的余地**——等于假设了 Entra 不具备的能力。规范自己在改（[#1614](https://github.com/modelcontextprotocol/modelcontextprotocol/issues/1614)：把 resource 改成 SHOULD + 回退 scope） |
| **Entra** | **从没实现 RFC 8707**（v2 是 scope-centric）；2026-03 的强制校验把过去默默容忍的冲突变成硬报错。不是"坏了"，是"本来就不支持规范假设的那套" |
| **Claude Code** | **严格照规范办事**（发 `resource`）→ 撞墙。合规但不够宽容——VS Code 那种老实现不发 `resource` 反而能用；外加尾斜杠小 bug（#52871） |
| **FastMCP（本项目 server）** | 在 `RemoteAuthProvider`（直连 Entra）模式下基本是**旁观者**——只广告 metadata、只验 token，不在发 token 的链路里。但它**能出手修**（换 `AzureProvider` 代理模式，见 §7） |
...
| **等上游修** | MCP 规范 #1614 把 `resource` 改成可回退；Claude Code #55993/#52871 也在追 | 无时间表、当前无 flag 可关 |
...
Entra v2 又不接受任何 `resource` 参数——所以"去掉 `resource`"只能在**服务端代持 token 交换**时做，即 proxy 模式。
...
- **Entra v2**：一直 scope-centric，从不实现 RFC 8707。
